...
//###################################################################################
		var displayEvents = function (){
			$('.events').remove();
			$(".eventHeader").remove();
			var makeEventHeader = $('<h2 class="eventHeader">Local Storage</h2>');
			makeEventHeader.appendTo('#eventHeader');
			if(localStorage.length === 0){
...
//###################################################################################
//load couch data function
//###################################################################################
	$('#loadcouch').on("click", function(){
		$(".eventHeader").remove();
		$(".couchevents").remove();
		var makeEventHeader = $('<h2 class="eventHeader">Couch Data</h2>');
		makeEventHeader.appendTo('#couchHeader');
		$.ajax({
			url:'_view/events',
			dataType:'json',
			success: function(data){
				...
				$.each(data.rows, function(index, event){
					...
					var medname   = event.value.medname;
					var typename  = event.value.typename;
					var dosage    = event.value.dosage;
					var frequency = event.value.frequency;
					var date      = event.value.date;
					var notes     = event.value.notes;
						$(
							'<ul id="myCouchData" class="couchevents" data-role="listview" data-inset="true" data-theme="b">' +
								'<li> Name: ' + name + '</li>' +
								'<li> Medication Name: ' + medname + '</li>' +
								'<li> Medication Type: ' + typename + '</li>' +
								'<li> Dosage: ' + dosage + '</li>' +
								'<li> Frequency: ' + frequency + '</li>' +
								'<li> Date: ' + date + '</li>' +
								'<li> Notes: ' + notes + '</li>'
							).appendTo('#couchDisplay');
